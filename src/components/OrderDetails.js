...
const OrderDetails = () => {
    ...
    useEffect(() => {
        if (sellerGuardians) {
            ...
            let orders = [];
            sellerGuardians
                .map((guardian) => guardian.fields.Sellers
                    .map((seller) => seller.fields.Orders
                        .map((order) => orders.push(order.fields))));
            console.log('orders: ', orders);
            // let orders = [];
            // orders.push(sellers.map((seller) => seller.fields.Orders));
            // let chosenOrder = find(sellerGuardians, matchesProperty('orderID', focusedRecordID));
            // console.log("chosenorder: ", chosenOrder)
            // setViewThisOrder(fullFundraiserRecord);
        }
    }, [sellerGuardians])
